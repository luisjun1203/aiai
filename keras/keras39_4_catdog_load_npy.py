...
for i, id in enumerate(id_list):                    # enumerate : 
    id_list[i] = int(id.split('.')[0])
    
# id.split('.'): 파일 이름에서 확장자를 제거합니다. 예를 들어, '123.jpg'의 경우 '123'만 추출됩니다.
# int(id.split('.')[0]): 추출된 문자열을 정수로 변환합니다.
# id_list[i] = ...: 변환된 정수를 원래 목록에 다시 할당합니다. 이렇게 하면 파일 이름의 숫자 부분만 남게 됩니다.
...
y_submit.to_csv(path + "submission_01_24_1.csv", index=False)
