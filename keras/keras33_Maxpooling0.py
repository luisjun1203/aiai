...
model.add(Conv2D(8, kernel_size=(2, 2),input_shape = (10, 10, 1), strides=1, padding='same'))  # stride : 커널사이즈를 몇칸씩 띄고 계산할지 지정  #(N, 27, 27, 19)   # 19 : filter( out layer 이름)
model.add(MaxPooling2D(pool_size=(2, 2), strides=1))
model.add(Conv2D(filters=7, kernel_size=(2, 2)))               #(N,25,25,97)                      # padding='valid : default
...
model.fit(X_train, y_train, epochs=100, batch_size=300,verbose=2, validation_split=0.15)
