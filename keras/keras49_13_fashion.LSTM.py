...
import pandas as pd
from keras.layers import Dense, Conv2D, Flatten, Dropout, GlobalAveragePooling2D, LSTM
...
model.add(LSTM(19, return_sequences=True,          
               input_length = 28, input_dim = 28, activation='relu'))         
model.add(LSTM(9, ))                                                       
...
model.fit(X_train, y_train, epochs=21, batch_size=21,verbose=2, validation_split=0.2, callbacks=[es])
end_time = time.time()
