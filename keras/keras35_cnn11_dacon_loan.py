...
test_csv = np.array(test_csv).astype(np.float32)

print(X_test.shape)
print(y_test.shape)

rbs = RobustScaler()
X_train = rbs.fit_transform(X_train.reshape(-1, 4*3*1)).reshape(-1, 4, 3, 1)
X_test = rbs.fit_transform(X_test.reshape(-1, 4*3*1)).reshape(-1, 4, 3, 1)
...
model = Sequential()                    
model.add(Conv2D(24, kernel_size=(2, 2), input_shape=(4, 3, 1), activation='swish', strides=2, padding='same'))   
model.add(Conv2D(12, kernel_size=(3, 3), activation='swish', strides=1, padding='same'))   
model.add(Conv2D(23, kernel_size=(2, 2), activation='swish', strides=2, padding='same'))   
model.add(Conv2D(36, kernel_size=(3, 3), activation='swish', strides=1, padding='same'))   
