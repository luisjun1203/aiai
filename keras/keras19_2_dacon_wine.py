...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, shuffle=True, random_state=3, stratify=y)
...
es = EarlyStopping(monitor='acc', mode='max', patience=120, verbose=3, restore_best_weights=True)
model.fit(X_train, y_train, epochs=1000, batch_size=25, validation_split=0.15, callbacks=[es])
