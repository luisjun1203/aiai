...
x_train = torch.FloatTensor(x_train).unsqueeze(1).to(DEVICE)        # 이걸 해줘야지 torch data로 사용할 수 있음 // unsqueeze(1) 2 차원을 맞춰주는 것
y_train = torch.FloatTensor(y_train).unsqueeze(1).to(DEVICE)        # shape를 똑같이 해줘야 된다 // 똑같지 않으면 그냥 n 빵 때려서 중간값으로 1개가 들어가게 됨 이 때는 2가 들어가게 됨
...
result = model(torch.Tensor([[8]]).to(DEVICE))
print('4의 예측값은 :' , result.item() )


# 최종 loss :  3.0248820484003736e-13
# 4의 예측값은 : 7.999999046325684