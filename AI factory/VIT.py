...
input_shape = (CFG.image_size, CFG.image_size, 3) # 여기서 3은 RGB 채널 수를 의미합니다.
model_input = Input(shape=input_shape)
model_vit = ImageTransformer(CFG.image_size, CFG.patch_size, CFG.num_classes, CFG.batch_size, CFG.projection_dim, CFG.transformer_layers, CFG.num_heads, CFG.projection_dim)
model_output = model_vit(model_input) # 모델에 입력 레이어를 전달합니다.





# Keras Model로 전체 모델을 래핑합니다.
# complete_model = tensorflow.keras.Model(inputs=model_input, outputs=model_output)
