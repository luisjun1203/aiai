...
# Reduce
rlr = ReduceLROnPlateau(monitor='val_loss', patience=1, factor=0.7, mode='min', min_lr = 0.000001)
...
print('가중치 저장')
model.save_weights('C:\\_data\\AI factory\\train_output\\model_MultiResUNet_PPP_final_weights_please.h5')
...
# joblib.dump(y_pred_dict, 'C:\\_data\\AI factory\\train_output\\y_pred_03_24_01.pkl') 
# print("done")
...
# 기존에 사전 훈련된 모델 불러오기
pretrained_model = MultiResUnet(height=256, width=256, n_channels=3)
pretrained_model.load_weights('C:\\_data\\AI factory\\train_output\\checkpoint-MultiResUNet-PPP-epoch_57_please.hdf5')

n = len(pretrained_model.layers)
# 모델의 일부 층 고정 (옵션)
for layer in pretrained_model.layers[:n]:
    layer.trainable = False

# 새로운 출력 층 추가 또는 기존 출력 층 수정
# 여기서는 이진 분류를 위한 Dense 층을 추가하는 예시입니다.
...
# 모델 저장
model.save_weights('C:\\_data\\AI factory\\train_output\\model_MultiResUNet_PPP_final_pretrained_weights_please.h5')